...
PanoToCubemapPSO::PanoToCubemapPSO()
{
	...
	ComPtr<ID3DBlob> shaderPanoToCubemapCS = Utility::ShaderCompile(L"D:\\Files\\Code\\C++\\RTRender\\RTRender\\Render\\Shader\\PanoToCubemap_CS.hlsl", nullptr, "main", "cs_5_1");
	pipelineStateStream.pRootSignature = mRootSignature.GetRootSignature().Get();
	pipelineStateStream.CS = { 
		reinterpret_cast<BYTE*>(shaderPanoToCubemapCS->GetBufferPointer()),
		shaderPanoToCubemapCS->GetBufferSize()
	};
	...
	mDefaultUAV = Application::Get().AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 5);
