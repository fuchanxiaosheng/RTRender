...
{
	mWindow = window;
	mpImGuiCtx = ImGui::CreateContext();
	ImGui::SetCurrentContext(mpImGuiCtx);
	if (!mWindow || !ImGui_ImplWin32_Init(mWindow->GetWindowHandle()))
	{
		return false;
	}
	...
	mFontTexture = std::make_unique<Texture>(fontTextureDesc);
	mFontTexture->SetName(L"ImGui Font Texture");
	...
	commandList->CopyTextureSubresource(*mFontTexture, 0, 1, &subresourceData);
	commandList->GenerateMips(*mFontTexture);
	...
	mRootSignature = std::make_unique<RootSignature>(rootSignatureDescription.Desc_1_1, featureData.HighestVersion);
	...
	ComPtr<ID3DBlob> shaderImGUIVS = Utility::ShaderCompile(L"D:\\Files\\Code\\C++\\RTRender\\RTRender\\Render\\Shader\\ImGUI_VS.hlsl", nullptr, "main", "vs_5_0");
	ComPtr<ID3DBlob> shaderImGUIPS = Utility::ShaderCompile(L"D:\\Files\\Code\\C++\\RTRender\\RTRender\\Render\\Shader\\ImGUI_PS.hlsl", nullptr, "main", "ps_5_0");
	...
	pipelineStateStream.VS = { 
		reinterpret_cast<BYTE*>(shaderImGUIVS->GetBufferPointer()),
		shaderImGUIVS->GetBufferSize()
	};
	pipelineStateStream.PS = { 
		reinterpret_cast<BYTE*>(shaderImGUIPS->GetBufferPointer()),
		shaderImGUIPS->GetBufferSize()
	};
...
void GUI::Render(std::shared_ptr<CommandList> commandList, const RenderTarget& renderTarget)
{
	...
	commandList->SetShaderResourceView(RootParameters::FontTexture, 0, *mFontTexture, D3D12_RESOURCE_STATE_PIXEL_SHADER_RESOURCE);
...
void GUI::Destroy()
{
	if (mWindow)
	{
		...
		ImGui::DestroyContext(mpImGuiCtx);
		mpImGuiCtx = nullptr;
		mWindow.reset();
	}
}
