...
GenerateMipsPSO::GenerateMipsPSO()
{
	...
	CD3DX12_ROOT_PARAMETER1 rootParameters[GenerateMips::RootParameterNum];
	...
	CD3DX12_VERSIONED_ROOT_SIGNATURE_DESC rootSignatureDesc(GenerateMips::RootParameterNum, rootParameters, 1, &linearClampSampler);

	mRootSignature.SetRootSignatureDesc(rootSignatureDesc.Desc_1_1, featureData.HighestVersion);
	...
	ComPtr<ID3DBlob> shaderGenerateMipsCS = Utility::ShaderCompile(L"D:\\Files\\Code\\C++\\RTRender\\RTRender\\Render\\Shader\\GenerateMips_CS.hlsl", nullptr, "main", "cs_5_1");

	pipelineStateStream.pRootSignature = mRootSignature.GetRootSignature().Get();
	pipelineStateStream.CS = {
		reinterpret_cast<BYTE*>(shaderGenerateMipsCS->GetBufferPointer()),
		shaderGenerateMipsCS->GetBufferSize()
	...
	mDefaultUAV = Application::Get().AllocateDescriptors(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 4);
	...
	for (UINT i = 0; i < 4; i++)
	{
		...
		device->CreateUnorderedAccessView(nullptr, nullptr, &uavDesc, mDefaultUAV.GetDescriptorHandle(i));
	}
}